...
df = pd.read_csv(transform_path + 'Migrations/create_varieties.csv')
...
try:
    ...
    cursor = cnx.cursor()
    # Drop the varieties table if it exists
    try:
        # Disable foreign key checks
        cursor.execute("SET FOREIGN_KEY_CHECKS = 0")
        cursor.execute("DROP TABLE IF EXISTS varieties")
    ...
    # Insert data into the varieties table
    insert_query = """
